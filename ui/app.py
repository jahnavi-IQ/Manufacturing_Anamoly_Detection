...
def main():
    ...
    # Persist analysis across reruns (e.g. download button clicks)
    ...
    if uploaded_file is not None:
        ...
        # Show results from session (survives download-button reruns)
        if st.session_state.analysis_result is not None:
            result = st.session_state.analysis_result
            meta = st.session_state.analysis_meta
            start_time_str = meta.get("start_time", "")
            analysis_filename = meta.get("filename", uploaded_file.name)
                ...
                if feature_values and feature_names:
                    ...
                    with col1:
                        ...
                        st.download_button(
                            ...
                            data=csv,
                            file_name=f"pump_features_{analysis_filename.replace('.wav', '')}.csv",
                            mime="text/csv",
                            width='stretch',
                    ...
                    with col2:
                        ...
                        st.download_button(
                            ...
                            data=json_data,
                            file_name=f"pump_analysis_{analysis_filename.replace('.wav', '')}.json",
            ...
            with tab3:
                ...
                if recommendations:
                    for rec in recommendations:
                        if "URGENT" in rec or "Immediate" in rec:
                            st.error(rec)
                        elif "anomaly" in rec.lower() or "inspection" in rec.lower() or "Monitor" in rec:
                            st.warning(rec)
                        else:
                            st.success(rec)
                else:
